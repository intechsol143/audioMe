...
import MapboxGL, {Camera, offlineManager} from '@rnmapbox/maps';
import React, {useEffect, useRef, useState} from 'react';
import {getNearistSpot, checkTour, getPlaces, searchTours} from '@src/lib/api';
...
const index = ({route, navigation}) => {
  ...
  const getpacks = async () => {
    const getpack = await offlineManager.getPacks();
    console.log('offline packs', spot?.image, spot?.voice);
    // await offlineManager.mergeOfflineRegions(getpack[10].pack);
  };
  ...
  // console.log('spot in start', spot);
  var getDistances = function (p1, p2, d1, d2) {
  ...
  const progressListener = (offlineRegion, status) => {
    console.log('status', status.percentage / 100);
    setBarValue(status.percentage / 100);
  };
  ...
  // }, []);
  //   console.log('posts', Element.post);
  useEffect(() => {
    if (!runNow) {
      MapboxGL.offlineManager.createPack(
        {
          name: `offlinePackage${Date.now()}`,
  ...
  useEffect(() => {
    getPlaces({token, Id})
      ...
      .catch(error => {});
    checkTour({token, tour_id: Element.id}).then(res => {
      setAudioPath(res.data.local_path);
    });
  }, []);
  ...
  console.log('try now', intrupted);
  useEffect(() => {
    // Alert.alert(Element.latitude, Lat);
    // Alert.alert(Lat);

    dontGet && cuRRentlocation();
    console.log('intrupted in effect', intrupted);
    // setTimeout(() => {
  ...
  //   setIsSoundPlaying(false);
  // }, 1000);
  const checkPos = (lat, long) => {
    Element.post.map(item => {
      // console.log('parameter', lat, long, item.lat, item.long);
      const spot = getDistances(lat, long, item.lat, item.long);
      console.log('spot', spot);
      if (spot <= 15) {
        // console.log('i even came in less than 120');
        if (!pauseAudio) {
          setSpot(item);
          // setIsSoundPlaying(true);
          setPlayAudio(true);
  ...
  const Spotslist = () => {
    return Element.post.map((element: any) => {
      return (
        ...
        <MapboxGL.PointAnnotation
          coordinate={[element.long, element.lat]}
          id="pt-ann">
          <TouchableOpacity
            onPress={() => {
  ...
  const Spotslist1 = () => {
    return (
      <MapboxGL.PointAnnotation coordinate={[Lng, Lat]} id="pt-ann">
        <TouchableOpacity
          onPress={() => {
            // setShowCard(true), setEle(element);
          }}
      ...
      <View
        ...
        {barValue < 1 && (
          <View style={{alignSelf: 'center'}}>
            <Progress.Bar progress={barValue} width={200} />
          </View>
        )}
          ...
          <View
            ...
            <View
              ...
              <Image
                ...
                source={{
                  uri: `file://${audioPath}/https://audiome.ca/app/public/post/image/${spot?.image?.slice(
                    spot?.image?.lastIndexOf('/') + 1,
                  )}`,
                  // spot.image,
                }}
              />
              ...
              <View style={[styles.row1, {marginHorizontal: 20}]}>
                ...
                <Video
                  ref={playerRef}
                  source={{
                    uri: `${audioPath}/https://audiome.ca/app/public/post/voice/${spot?.voice?.slice(
                      spot?.voice?.lastIndexOf('/') + 1,
                    )}`,
                  }}
